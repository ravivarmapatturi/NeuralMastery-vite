...
# Specific key topics required by user
SPECIFIC_TOPICS = {
    "agent-loop": [
...
def generate_curriculum():
    ...
    for track in TRACKS:
        ...
        # Determine topics for this track
        ...
        for i in range(1, count + 1):
            ...
            if topic_pool and (i - 1) < len(topic_pool):
                title = topic_pool[i - 1]
            else:
                title = f"{track_name} Module {((i - 1) // 5) + 1}: Problem {i}"

            # Calculate stage
            stage_num = min(8, ((global_rank - 1) // 63) + 1)
            stage_names = [
                ...
                "Stage 8: Evaluation, Security & Production Deployment"
            ]
            ...
            if global_rank == 500:
                difficulty = "hard"
                points = 150
                title = "BUILD A PRODUCTION AGENT"
    ...
    Implement solution for {title} in the Agentic AI stack.
    \"\"\"
    # Implement core agentic logic here
    pass
"""
...
description: "Master {title} in the {track_name} track of the Agentic AI Engineering curriculum."
difficulty: "{difficulty}"
topic: "{track_name}"
---
...
**Track**: {track_name} | **Rank**: #{global_rank} of 500 | **Points**: {points} XP

## Overview

Implement `{title}` to advance your mastery in **{track_name}**. This problem is part of Neural Mastery's 500-problem Agentic AI Engineering curriculum.
...
"""
    ...
    print(f"Generated {len(problems)} Agentic AI curriculum problems.")
