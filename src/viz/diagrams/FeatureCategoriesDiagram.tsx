    ...
    <VisualizationContainer footer={active.desc}>
      <div style={{ display: 'flex', gap: 8 }}>
        {CATEGORIES.map((c) => {
          ...
          return (
            <div key={c.key} onClick={() => setSelected(c.key)} onMouseEnter={() => setSelected(c.key)} style={{ flex: 1, cursor: 'pointer', padding: '0.7rem', borderRadius: 9, background: isSelected ? `${color}18` : t.surfaceAlt, border: `1.5px solid ${isSelected ? color : t.border}`, textAlign: 'center' }}>
