  ...
  return (
    ...
    >
      <div style={{ display: 'flex', gap: 8 }}>
        <div onClick={() => setActive('monitoring')} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setActive('monitoring'); } }} onMouseEnter={() => setActive('monitoring')} style={{ flex: 1, cursor: 'pointer', padding: '0.8rem', borderRadius: 9, background: active === 'monitoring' ? `${monColor}18` : t.surfaceAlt, border: `1.5px solid ${active === 'monitoring' ? monColor : t.border}`, textAlign: 'center' }}>
        ...
        <div onClick={() => setActive('observability')} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); setActive('observability'); } }} onMouseEnter={() => setActive('observability')} style={{ flex: 1, cursor: 'pointer', padding: '0.8rem', borderRadius: 9, background: active === 'observability' ? `${obsColor}18` : t.surfaceAlt, border: `1.5px solid ${active === 'observability' ? obsColor : t.border}`, textAlign: 'center' }}>
