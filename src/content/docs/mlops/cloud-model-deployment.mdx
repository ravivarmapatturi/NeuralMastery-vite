...
description: "Real, concrete methods for deploying a trained model on AWS and Azure specifically -- SageMaker endpoints, Bedrock, Azure ML, with real commands, not just names."
---
...
<GoDeeper title="Go deeper: what a 'cold start' actually is, in order">

"Cold-start latency" isn't one delay, it's a real sequence of them, each with a different cause and a different fix: (1) provisioning compute — SageMaker has to allocate a container instance that wasn't already running, on the order of seconds; (2) pulling the container image — if it isn't already cached on that host, this can dominate the whole cold start for large images (a multi-gigabyte deep learning image with CUDA/cuDNN baked in is common); (3) loading the model into memory (or GPU memory) — reading model weights off disk/network storage and initializing the framework's runtime; (4) framework warm-up — some inference engines JIT-compile or build internal caches on the first real request, making even the *first served request* slower than the steady-state ones after it. `ProvisionedConcurrency` above doesn't eliminate any of these steps — it pre-pays for them ahead of time, keeping that many containers already through all four stages and idling warm, so only requests beyond that concurrency level pay the real cold-start cost.
