...
sidebar_position: 20
description: "The real model-to-container-to-GPU-VM-to-API hosting stack, the fine-tuning-to-serving handoff, and the LLM-specific monitoring layer on top."
---
...
```bash
ollama pull llama3.1:8b       # downloads and stores the GGUF weights locally
ollama run llama3.1:8b        # interactive chat, or pipe a prompt in non-interactively
ollama serve                  # starts Ollama's own local API on :11434
curl http://localhost:11434/api/generate -d '{"model": "llama3.1:8b", "prompt": "Explain KV caching in one sentence."}'
```
...
vllm serve meta-llama/Llama-3.1-8B-Instruct \
  --enable-lora \
  --lora-modules support-tone=./adapters/support-tone billing-tone=./adapters/billing-tone
```

```bash
# a request picks the adapter by name in the `model` field -- same base weights underneath
curl http://localhost:8000/v1/completions -d '{"model": "support-tone", "prompt": "..."}'
```
...
curl http://localhost:8081/embed -H "Content-Type: application/json" \
  -d '{"inputs": "How does prefix caching work?"}'
```
