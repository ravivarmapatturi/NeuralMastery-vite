...
- [ ] AWS: SageMaker real-time endpoints, Serverless Inference, Async Inference, Batch Transform, multi-model/multi-container endpoints
- [ ] AWS container-native: ECS, EKS, Lambda; Bedrock for managed LLMs
- [ ] Azure ML: managed online endpoints, batch endpoints
- [ ] Azure container-native: AKS, Container Apps, Container Instances; Azure OpenAI Service for managed LLMs
...
- [ ] FedAvg: the round cycle (distribute, local train, upload, weighted aggregate), and the C/E/B hyperparameters
- [ ] Non-IID, unbalanced, massively-distributed, limited-communication -- why this isn't just "distributed training but slower"
- [ ] Differential privacy (central vs. local) and gradient clipping; secure aggregation
- [ ] Federated fine-tuning of LLMs with LoRA/PEFT updates instead of full weights
- [ ] Flower (`flwr`): ClientApp/ServerApp, the FedAvg strategy, `flwr run`
