...
sidebar_position: 4
description: "The real evolution from a single prompt to a full agent graph -- ReAct, plan-and-execute, and every architecture's tradeoff between reliability and cost."
---
...
- **Graph** — multiple loops, tool calls, and sub-agents become nodes; the edges between them (including *conditional* edges) carry shared state. This is the [Supervisor Pattern](/docs/agents/routing-and-supervisor#the-actual-difference-not-just-the-definitions) below, and what a framework like LangGraph makes concrete: an explicit state machine instead of one long linear transcript. For the actual node types, real-vs-fake edges, and how rejections get routed, see [Loops and Graphs: The Actual Anatomy](/docs/agents/loops-and-graphs).
