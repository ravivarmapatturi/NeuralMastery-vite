...
For an agent running 10 iterations with a 2,000-token system prompt and 500 tokens per step, the total token consumption is not $2,000 + 10 \times 500 = 7,000$ tokens — it is **29,500 tokens**. Cost and time-to-first-token (TTFT) both scale with this quadratic curve.
