...
**Why independence, and why it works**: ICA relies on the Central Limit Theorem (see [Probability & Statistics](/docs/mathematics-for-ai/probability-statistics#central-limit-theorem)) in reverse — a mixture of independent signals tends to look *more* Gaussian than any of the original signals alone (sums of independent random variables trend toward Gaussian). So ICA searches for a transformation that makes the resulting components as **non-Gaussian** as possible, on the logic that this is exactly what un-mixes the original signals back out.
