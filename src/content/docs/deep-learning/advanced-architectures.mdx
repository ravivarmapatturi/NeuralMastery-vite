...
- **Masked prediction SSL (BERT for text, MAE for images)**: mask out part of the input and train the model to reconstruct or predict the missing part — BERT's masked language modeling (see [Attention & Transformers](/docs/deep-learning/attention-transformers)) and the **Masked Autoencoder (MAE)** for images are the same underlying principle applied to different modalities.
