...
sidebar_position: 7
description: "Evaluating LLMs beyond perplexity and benchmarks -- LLM-as-judge, position bias -- plus serving: continuous batching, speculative decoding, and cost tradeoffs."
---
...
**Reasoning models add a fourth axis**: the internal "thinking" tokens a reasoning model generates before its visible answer are real billed output tokens, not free — see [Prompt Engineering — Test-Time Compute Scaling](/docs/llms-genai/prompt-engineering#test-time-compute-scaling-a-second-scaling-axis). Evaluating one at scale costs meaningfully more per run than the visible response length suggests, since the hidden reasoning trace is often many times longer than the final answer.
