...
**What "the embedding model" actually is:** every bi-encoder on this page is an [SBERT (Sentence-BERT)](/docs/nlp/word-embeddings#from-contextual-tokens-to-a-single-sentence-vector-sentence-bert)-style model — a pretrained encoder with mean pooling, fine-tuned specifically so cosine similarity becomes meaningful. That's not an implementation detail; it's the entire reason bi-encoder embeddings work for retrieval at all.
