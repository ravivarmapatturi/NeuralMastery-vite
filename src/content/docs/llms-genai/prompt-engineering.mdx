...
In production, this is usually a **routing** decision (see [Agent Architectures — Routing](/docs/agents/agent-architectures#routing)): classify or detect whether a request is the kind that benefits from extended reasoning, and only pay the latency/cost premium for the requests that actually need it, rather than defaulting every request to the most expensive mode available.
